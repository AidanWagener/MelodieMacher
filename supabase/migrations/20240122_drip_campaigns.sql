-- Drip Campaign System for MelodieMacher
-- Automated email sequences for customer engagement
...
-- Campaign Steps (emails in the sequence)
...
-- Email Sends (tracking all sent emails)
...
-- Campaign Enrollments (users enrolled in campaigns)
...
-- Insert default drip campaigns
INSERT INTO drip_campaigns (name, slug, description, trigger_event) VALUES
(
  'Post-Delivery Engagement',
  'post-delivery',
  'Automatische Follow-up Emails nach Lieferung',
  'order_delivered'
),
(
  'Anniversary Re-engagement',
  'anniversary',
  'Erinnerung ein Jahr nach der Bestellung',
  'order_anniversary'
);

-- Insert campaign steps for Post-Delivery campaign
...
-- Insert campaign step for Anniversary campaign
INSERT INTO campaign_steps (campaign_id, template_id, step_order, delay_days, condition_type)
SELECT
  c.id,
  t.id,
...
-- Add priority columns to orders if they don't exist
DO $$
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'priority_reasons') THEN
    ALTER TABLE orders ADD COLUMN priority_reasons TEXT[];
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'suggested_deadline') THEN
    ALTER TABLE orders ADD COLUMN suggested_deadline TIMESTAMPTZ;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'generated_prompt') THEN
